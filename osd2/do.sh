...
qemu-img create -f qcow2 $IMAGES/$hn.vdb.qcow2 20G
qemu-img create -f qcow2 $IMAGES/$hn.vdc.qcow2 20G
qemu-img create -f qcow2 $IMAGES/$hn.vdd.qcow2 20G
qemu-img create -f qcow2 $IMAGES/$hn.vde.qcow2 20G
...
--disk path=$IMAGES/$hn.vdc.qcow2 \
--disk path=$IMAGES/$hn.vdd.qcow2 \
--disk path=$IMAGES/$hn.vde.qcow2 \
...
EOF
virsh attach-device $hn if1.xml --config
